...
const int js0PinSw = 2;  // Цифра - Кнопка джойстика №0
const int btnBTCall = 3; // Цифра - Прием/сброс звонка на блютуз-модуле
const int btnBTVUp = 4; // Цифра - Увеличение громкости на блютуз-модуле
const int btnBTVDn = 5;  // Цифра - Уменьшение громкости на блютуз-модуле
...
boolean statStart = false; // определение перезапуска(0 - система только что запущена, 1 - прошел период инициализации устройств)
boolean statBt = false; //состояние блютуз-модуля
boolean statLan = false; //состояние LAN-модуля
boolean statD = false; //движение вперед в данный момент времени (ДРАЙВ)
boolean statN = true; //движение отсуствует в данный момент времени  (!!!НЕЙТРАЛЬНАЯ!!! по умолчанию включена)
...
int js0TempX = 512; //инициализируется значением оси X только при нажатии соответствующей ей кнопки
int js0TempY = 700; //инициализируется значением оси Y только при нажатии соответствующей ей кнопки, также выполняет требования входа в начальное меню после инициализации
...
int allAudioVol = 0; //общая громкость аудиосистемы
int timeDow = 0; //инициализируется значением дня недели в данный момент времени
int timeHour = 0; //инициализируется значением часа в данный момент времени
int timeMin = 0; //инициализируется значением минуты в данный момент времени
int timeSec = 0; //инициализируется значением секунды в данный момент времени
int statError = 4; //инициализируется значением количества ошибок в данный момент времени
int preloader = 0; //хранит конечное время отображения прелоадера
...
} // Обработка показаний джойстика - конец
...
void fPreloader () // Функция вывода прелоадера на дисплей - начало
{
  ...
  do {
    ...
    if (preloader == 0 && statStart == false) // Дополнительно исключаем занос 55+5=0 вторым параметром
    {
  ...
  while (u8g.nextPage());
  fBT(3);
} // Функция вывода прелоадера на дисплей - конец



void fMenuStat() // Верхнее меню - начало
{
  ...
  if (statBt == false) // Статус сети BT - ачало
  {
    u8g.setColorIndex(1);
...
void fMenu() // Основное меню - начало
{
  *pMenuTemp = 0;
...
void fMenuUp() // Верхнее меню - начало
{
  *pMenuTemp = 3;
  u8g.setFont(rus4x6);
...
void fScreen () // Функция вывода на дисплей основного меню - начало
{
  ...
  do {
    fMenuStat();
    fBT(0);
    if (js0TempY > 650) fMenu();  // Основное меню - корень
    ...
    if ((menuTemp == 0 || menuTemp == 3) && js0TempX < 450) fMenuUp(); // Верхнее меню - корень
...
void setup()
{
  pinMode(js0PinSw, INPUT); // Цифра - кнопка джойстика №0
  digitalWrite(js0PinSw, HIGH); // Включаем встроенный подтягивающий резистор, значение на пине всегда "1"
  pinMode(btnBTCall, OUTPUT); // Цифра - прием/сброс звонка на блютуз-модуле
  pinMode(btnBTVUp, OUTPUT); // Цифра - увеличение громкости на блютуз-модуле
  pinMode(btnBTVDn, OUTPUT);  // Цифра - уменьшение громкости на блютуз-модуле
